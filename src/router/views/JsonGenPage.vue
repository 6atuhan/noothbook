




<template>
    
    <div class="m-4">
        <p v-for="kelime,index,i in fiillerimiz" :key="kelime">
        {{i+1}}<br>&nbsp;&nbsp;ing = {{index}}<br>&nbsp;&nbsp;tr = {{kelime}}<br>&nbsp;&nbsp;sonuç b = false<br>&nbsp;&nbsp;id number = {{i+1}}
    </p>
    </div>
  </template>
  <script setup>
    import { ref } from "vue"
  
    const fiillerimiz = {
  "be": "olmak",
  "have": "sahip olmak",
  "do": "yapmak",
  "say": "söylemek",
  "get": "almak",
  "make": "yapmak",
  "go": "gitmek",
  "see": "görmek",
  "know": "bilmek",
  "take": "almak",
  "think": "düşünmek",
  "come": "gelmek",
  "give": "vermek",
  "look": "bakmak",
  "use": "kullanmak",
  "find": "bulmak",
  "want": "istemek",
  "tell": "söylemek",
  "put": "koymak",
  "mean": "kastetmek",
  "become": "olmak",
  "leave": "bırakmak",
  "work": "çalışmak",
  "need": "istemek",
  "feel": "hissetmek",
  "seem": "görünmek",
  "ask": "sormak",
  "show": "göstermek",
  "try": "denemek",
  "call": "aramak",
  "keep": "tutmak",
  "provide": "sağlamak",
  "hold": "tutmak",
  "turn": "dönmek",
  "follow": "takip etmek",
  "begin": "başlamak",
  "bring": "getirmek",
  "like": "hoşlanmak",
  "help": "yardım etmek",
  "start": "başlamak",
  "run": "koşmak",
  "write": "yazmak",
  "set": "kurmak",
  "move": "ilerlemek",
  "play": "oynamak",
  "pay": "ödemek",
  "hear": "duymak",
  "include": "katmak",
  "believe": "inanmak",
  "allow": "izin vermek",
  "meet": "tanışmak",
  "lead": "yönlendirmek",
  "live": "yaşamak",
  "stand": "durmak",
  "happen": "olmak",
  "carry": "taşımak",
  "talk": "konuşmak",
  "appear": "ortaya çıkmak",
  "produce": "üretmek",
  "sit": "oturmak",
  "offer": "teklif etmek",
  "consider": "düşünmek",
  "expect": "beklemek",
  "suggest": "önermek",
  "let": "izin vermek",
  "read": "okumak",
  "require": "istemek",
  "continue": "devam etmek",
  "lose": "kaybetmek",
  "add": "eklemek",
  "change": "değiştirmek",
  "fall": "düşmek",
  "remain": "kalmak",
  "remember": "hatırlamak",
  "buy": "satın almak",
  "speak": "konuşmak",
  "stop": "durmak",
  "send": "göndermek",
  "receive": "almak",
  "decide": "karar vermek",
  "win": "kazanmak",
  "understand": "anlamak",
  "describe": "açıklamak",
  "develop": "geliştirmek",
  "agree": "katılmak",
  "open": "açmak",
  "reach": "yetişmek",
  "build": "inşa etmek",
  "involve": "içermek",
  "spend": "harcamak",
  "return": "dönmek",
  "draw": "çizmek",
  "die": "ölmek",
  "hope": "umutlanmak",
  "create": "yaratmak",
  "walk": "yürümek",
  "sell": "satmak",
  "wait": "beklemek",
  "cause": "sebep olmak",
  "pass": "geçmek",
  "lie": "yalan söylemek",
  "accept": "kabul etmek",
  "watch": "izlemek",
  "raise": "yükselmek",
  "base": "dayandırmak",
  "apply": "uygulamak",
  "break": "kırmak",
  "explain": "açıklamak",
  "learn": "öğrenmek",
  "increase": "artmak",
  "cover": "örtmek",
  "grow": "büyümek",
  "claim": "istemek",
  "report": "bildirmek",
  "support": "desteklemek",
  "cut": "kesmek",
  "form": "oluşturmak",
  "stay": "kalmak",
  "contain": "kapsamak",
  "reduce": "azaltmak",
  "establish": "kurmak",
  "join": "katılmak",
  "wish": "dilemek",
  "achieve": "başarmak",
  "seek": "aramak",
  "choose": "seçmek",
  "deal": "anlaşmak",
  "face": "yüzleşmek",
  "fail": "başarısız olmak",
  "serve": "servis etmek",
  "end": "bitirmek",
  "kill": "öldürmek",
  "occur": "olmak",
  "drive": "kullanmak",
  "represent": "anlatmak",
  "rise": "yükselmek",
  "discuss": "tartışmak",
  "love": "sevmek",
  "pick": "seçmek",
  "place": "yerleştirmek",
  "argue": "tartışmak",
  "prove": "kanıtlamak",
  "wear": "giymek",
  "catch": "yakalamak",
  "enjoy": "keyfine varmak",
  "eat": "yemek",
  "introduce": "tanıtmak",
  "enter": "girmek",
  "present": "sunmak",
  "arrive": "varmak",
  "ensure": "sağlamak",
  "point": "göstermek",
  "plan": "planlamak",
  "pull": "çekmek",
  "refer": "anmak",
  "act": "davranmak",
  "relate": "ilgilenmek",
  "affect": "etkilemek",
  "close": "kapatmak",
  "identify": "tanımak",
  "manage": "yönetmek",
  "thank": "teşekkür etmek",
  "compare": "karşılaştırmak",
  "announce": "sunmak",
  "obtain": "edinmek",
  "note": "not almak",
  "forget": "unutmak",
  "indicate": "belirtmek",
  "wonder": "merak etmek",
  "maintain": "korumak",
  "publish": "yayınlamak",
  "suffer": "acı çekmek",
  "avoid": "kaçmak",
  "express": "açıklamak",
  "suppose": "sanmak",
  "finish": "bitirmek",
  "determine": "belirtmek",
  "design": "tasarlamak",
  "listen": "dinlemek",
  "save": "kaydetmek",
  "tend": "ilgilenmek",
  "treat": "iyileştirmek",
  "control": "kontrol etmek",
  "share": "paylaşmak",
  "remove": "çıkarmak",
  "throw": "atmak",
  "visit": "ziyaret etmek",
  "exist": "var olmak",
  "encourage": "cesaretlendirmek",
  "force": "zorlamak",
  "reflect": "yansıtmak",
  "admit": "itiraf etmek",
  "assume": "varsaymak",
  "smile": "gülmek",
  "prepare": "hazırlanmak",
  "replace": "yerine koymak",
  "fill": "doldurmak",
  "improve": "gelişmek",
  "mention": "bahsetmek",
  "fight": "savaşmak",
  "intend": "istemek",
  "miss": "kaçırmak",
  "discover": "keşfetmek",
  "drop": "süşmek",
  "hit": "vurmak",
  "push": "ittirmek",
  "prevent": "engellemek",
  "refuse": "itiraz etmek",
  "regard": "saymak",
  "lay": "sermek",
  "reveal": "açıklamak",
  "teach": "öğretmek",
  "answer": "cevaplamak",
  "operate": "çalıştırmak",
  "state": "belirtmek",
  "depend": "bağlanmak",
  "enable": "izin vermek",
  "record": "kaydetmek",
  "check": "bakmak",
  "complete": "tamamlamak",
  "cost": "mal olmak",
  "sound": "söylemek",
  "laugh": "gülmek",
  "realise": "farketmek",
  "extend": "uzatmak",
  "arise": "ortaya çıkmak",
  "notice": "farketmek",
  "define": "açıklamak",
  "examine": "incelemek",
  "fit": "sığmak",
  "study": "çalışmak",
  "bear": "taşımak",
  "hang": "asmak",
  "recognise": "tanımak",
  "shake": "sallamak",
  "sign": "imzalamak",
  "attend": "katılmak",
  "fly": "uçmak",
  "gain": "kazanmak",
  "perform": "yapmak",
  "result": "sonuçlanmak",
  "travel": "seyahat etmek",
  "adopt": "sahip çıkmak",
  "confirm": "onaylamak",
  "protect": "korumak",
  "demand": "istmek",
  "stare": "bakmak",
  "imagine": "hayal etmek",
  "attempt": "denemek",
  "beat": "dövmek",
  "born": "doğmak",
  "associate": "birleştirmek",
  "care": "ilgilenmek",
  "marry": "evlenmek",
  "collect": "toplamak",
  "voice": "seslendirmek",
  "employ": "iş vermek",
  "issue": "bildirmek",
  "release": "açıklamak",
  "emerge": "ortaya çıkmak",
  "mind": "önemsemek",
  "aim": "hedeflemek",
  "deny": "inkar etmek",
  "mark": "işaretlemek",
  "shoot": "vurmak",
  "appoint": "ayarlamak",
  "order": "emretmek",
  "supply": "sağlamak",
  "drink": "içmek",
  "observe": "incelemek",
  "reply": "yanıtlamak",
  "ignore": "önemsememek",
  "link": "bağlamak",
  "propose": "teklif etmek",
  "ring": "çalmak",
  "settle": "yerleşmek",
  "strike": "vurmak",
  "press": "bastırmak",
  "respond": "cevap vermek",
  "arrange": "ayarlamak",
  "survive": "sağ kalmak",
  "concentrate": "odaklanmak",
  "lift": "kaldırmak",
  "approach": "yaklaşmak",
  "cross": "geçmek",
  "test": "test etmek",
  "charge": "yüklemek",
  "experience– deneyimlemek": {},
  "touch": "dokunmak",
  "acquire": "edinmek",
  "commit": "işlemek",
  "demonstrate– göstermek": {},
  "grant": "izin vermek",
  "prefer": "tercih etmek",
  "repeat": "tekrar etmek",
  "sleep": "uyumak",
  "threaten": "tehdit etmek",
  "feed": "beslemek",
  "insist": "ısrar etmek",
  "launch": "başlatmak",
  "limit": "sınırlamak",
  "promote": "desteklemek",
  "deliver": "göndermek",
  "measure": "ölçmek",
  "own": "sahip olmak",
  "retain": "sürdürmek",
  "assess": "hesaplamak",
  "attract": "cezbezmek",
  "belong": "uygun olmak",
  "consist": "içermek",
  "contribute": "destek olmak",
  "hide": "saklanmak",
  "promise": "söz vermek",
  "reject": "reddetmek",
  "cry": "ağlamak",
  "impose": "dayatmak",
  "invite": "davet etmek",
  "sing": "şarkı söylemek",
  "vary": "çeşitlendirmek",
  "warn": "uyarmak",
  "address": "hitap etmek",
  "declare": "ilan etmek",
  "destroy": "yok etmek",
  "worry": "endişelenmek",
  "divide": "ayrılmak",
  "head": "kullanmak",
  "name": "ad vermek",
  "stick": "batırmak",
  "nod": "seçilmek",
  "recognize": "tanımak",
  "train": "eğitmek",
  "attack": "saldırmak",
  "clear": "temizlemek",
  "combine": "birleştirmek",
  "handle": "halletmek",
  "influence": "etkilemek",
  "realize": "fark etmek",
  "recommend": "önermek",
  "shout": "bağırmak",
  "spread": "dağıtmak",
  "undertake": "üstlenmek",
  "account": "açıklamak",
  "select": "seçmek",
  "climb": "tırmanmak",
  "contact": "temas etmek",
  "recall": "hatırlamak",
  "secure": "korumak",
  "step": "adım atmak",
  "transfer": "yollamak",
  "welcome": "karşılamak",
  "conclude": "sonuçlandırmak",
  "disappear": "kaybolmak",
  "display": "göstermek",
  "dress": "giyinmek",
  "illustrate": "anlatmak",
  "imply": "ima etmek",
  "organise": "organize etmek",
  "direct": "yönlendirmek",
  "escape": "kaçmak",
  "generate": "oluşturmak",
  "investigate": "araştırmak",
  "remind": "hatırlamak",
  "advise": "önermek",
  "afford": "karşılamak",
  "earn": "kazanmak",
  "hand": "vermek",
  "inform": "haber vermek",
  "rely": "güvenmek",
  "succeed": "başarmak",
  "approve": "onaylamak",
  "burn": "yanmak",
  "fear": "korkmak",
  "vote": "oy vermek",
  "conduct": "yönetmek",
  "cope": "başa çıkmak",
  "derive": "türemek",
  "elect": "seçmek",
  "gather": "toplamak",
  "jump": "zıplamak",
  "last": "sürmek (süre)",
  "match": "eşleştirmek",
  "matter": "önemli olmak",
  "persuade": "ikna etmek",
  "ride": "binmek",
  "shut": "kapatmak",
  "blow": "üflemek",
  "estimate": "tahmin etmek",
  "recover": "iyileşmek",
  "score": "puan almak",
  "slip": "kaymak",
  "count": "saymak",
  "hate": "nefret etmek",
  "attach": "eklemek",
  "exercise": "egzersiz yapmak",
  "house": "yerleşmek",
  "lean": "yaslanmak",
  "roll": "yuvarlamak",
  "wash": "yıkamak",
  "accompany": "katılmak",
  "accuse": "suçlamak",
  "bind": "bağlamak",
  "explore": "keşfetmek",
  "judge": "yargılamak",
  "rest": "dinlenmek",
  "steal": "çalmak",
  "comment": "yorumlamak",
  "exclude": "dışlamak",
  "focus": "odaklanmak",
  "hurt": "incinmek",
  "stretch": "gerinmek",
  "withdraw": "çekmek",
  "back": "yardım etmek",
  "fix": "onarmak",
  "justify": "savunmak",
  "knock": "vurmak",
  "pursue": "devam ettirmek",
  "switch": "değiştirmek",
  "appreciate": "takdir etmek",
  "benefit": "yararlanmak",
  "lack": "yoksun kalmak",
  "list": "sıralamak",
  "occupy": "işgal etmek",
  "permit": "izin vermek",
  "surround": "etrafını sarmak",
  "abandon": "terk etmek",
  "blame": "suçlamak",
  "complain": "yakınmak",
  "connect": "bağlanmak",
  "construct": "inşa etmek",
  "dominate": "baskılamak",
  "engage": "bağlamak",
  "paint": "boyamak",
  "quote": "alıntı",
  "view": "görüntülemek",
  "acknowledge": "kabullenmek",
  "dismiss– uzaklaştırmak": {},
  "incorporate": "birleştirmek",
  "interpret": "yorumlamak",
  "proceed": "devam etmek",
  "search": "aramak",
  "separate": "ayırmak",
  "stress": "gerilmek",
  "alter": "değiştirmek",
  "analyse": "analiz etmek",
  "arrest": "tutuklamak",
  "bother": "rahatsız etmek",
  "defend": "savunmak",
  "expand": "genişletmek",
  "implement": "uygulamak",
  "possess": "sahip olmak",
  "review": "değerlendirmek",
  "suit": "uymak",
  "tie": "bağlamak",
  "assist": "yardım etmek",
  "calculate": "hesaplamak",
  "glance": "bakmak",
  "mix": "karıştırmak",
  "question": "sorgulamak",
  "resolve": "çözümlemek",
  "rule": "kural",
  "suspect": "şüphelenmek",
  "Wake": "uynamak",
  "appeal": "başvurmak",
  "challenge": "meydan okumak",
  "clean": "temizlemek",
  "damage": "zedelemek",
  "guess": "tahmin etmek",
  "reckon": "sanmak",
  "restore": "yenilemek",
  "restrict": "sınırlamak",
  "specify": "belirtmek",
  "constitute": "oluşturmak",
  "convert": "çevirmek",
  "distinguish": "ayırmak",
  "submit": "göndermek",
  "trust": "güvenmek",
  "urge": "ısrar etmek",
  "feature": "yayınlamak",
  "land": "karaya çıkmak",
  "locate": "yerleşmek",
  "predict": "tahmin etmek",
  "preserve": "korumak",
  "solve": "çözmek",
  "sort": "düzenlemek",
  "struggle": "zorlanmak",
  "cast": "dökmek",
  "cook": "pişirmek",
  "dance": "dans etmek",
  "invest": "yatırım",
  "lock": "kilitlemek",
  "owe": "borçlanmak",
  "pour": "dökmek",
  "shift": "değiştirmek",
  "kick": "tekme atmak",
  "kiss": "öpmek",
  "light": "yakmak",
  "purchase": "satın almak",
  "race": "yarışmak",
  "retire": "emekli olmak"
  }
  
   console.log('fiillerimiz', fiillerimiz)
  
    
    //https://beta5.objgen.com/json/local/design
  </script>